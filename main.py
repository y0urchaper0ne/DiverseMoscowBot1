...
from text import bolshoi_history_text, bolshoi_building_text, bolshoi_history_url, rules, rules_url
...
def wake_up(update, context):
    ...
    context.bot.send_message(
        chat_id=chat.id,
        text='Привет, {}! Давай начнем наше театральное приключение. Ты можешь сначала прочитать правила или же сразу приступить к квесту. Выбери, что хочешь👇'.format(name),
        reply_markup=button
    )
    return 'INTRO'
...
def history_quizz_menu_callback(update, context):
    ...
    if query.data == 'hint':
        ...
        query.edit_message_text(
            text=f"Не подскажете, под каким названием театр прослужил меньше всего? \n\nПиши ответ внизу 👇")
        query.message.reply_text(
            text=f'<tg-spoiler>Большой Петровский театр</tg-spoiler>', parse_mode='HTML')
